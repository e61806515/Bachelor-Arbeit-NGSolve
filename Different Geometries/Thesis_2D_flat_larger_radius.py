...
geo = MakeGeometry(d_M=d_M, delta = delta_rot, d_L=2e-3, tau_p=tau_p, tau=tau)
Draw(geo)
...
try:
    with open("simulations.txt", "a") as file:
        file.write(f"Flat Sim: PZ = {PZ}, Tau = {tau}, omega = {omega}, A_magnet = {A}: losses = {PZ*losses.real}, delta_magnet = {delta_mag}, Innenradius = {tau_p/(2*pi)*PZ-3e-3} \n")
        print("written to file")
except Exception as e:
