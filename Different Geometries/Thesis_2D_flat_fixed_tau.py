...
def MakeGeometry(d_M, delta, d_L, tau_p, tau):
        ...
        if tau < 1:
                ...
                geo.Append(["line", pnums[4], pnums[3]], leftdomain=2, rightdomain=1, bc="magnet_left", maxh = 0.1*delta_rot)
                geo.Append(["line", pnums[6], pnums[5]], leftdomain=2, rightdomain=1, bc="magnet_right", maxh = 0.1*delta_rot)
        ...
        geo.SetDomainMaxH(1, 0.1*delta_rot)
        geo.SetDomainMaxH(2,0.1*delta_rot)
        geo.SetDomainMaxH(3, 0.1*delta_rot)
        return geo
...
print("Berechnete Fläche ist ", A_round*PZ)
d_L = 2e-3
d_M = 3*d_L
...
geo = MakeGeometry(d_M=d_M, delta = delta_rot, d_L=d_L, tau_p=tau_p, tau=tau)
Draw(geo)
print(geo.GetNSplines())
mesh = geo.GenerateMesh(maxh = 0.1*delta_rot)
mesh = Mesh(mesh)
...
try:
    with open("simulations.txt", "a") as file:
        file.write(f"Flat Sim tau_p: PZ = {PZ}, Tau = {tau}, omega = {omega}, A_magnet = {A*PZ}: losses = {losses.real}, delta_magnet = {delta_mag} \n")
        print("written to file")
except Exception as e:
