...
def MakeGeometry(H_L, H_M, H_Fe, H_W, Tau, PZ):
    ...
    magnets_real = magnets[0:3] + magnets[10:12]

    #Nun ist die obere Hälfte der Magnete abgebildet (für 12), aber es fehlen noch die Ränder!
    ...
    geo = Glue(([outer-rotor - sum(magnets_real), rotor- inner] + magnets_real))
...
def K(x,y):
     a = IfPos(Phi(x,y)-10*pi*6, True, False)
     if(a):
          return -K0*exp(1j*2*pi/6)
     a = IfPos(Phi(x,y)-8*pi*6, True, False)
     if(a):
          return -K0*exp(1j*2*pi/3)
     a = IfPos(Phi(x,y)-6*pi*6, 1, 0)
     if(a):
          return -K0
     a = IfPos(Phi(x,y)-4*pi*6, 1, 0)
     if(a):
          return K0*exp(1j*4*pi/3)
     a = IfPos(Phi(x,y)-2*pi*6, 1, 0)
     if(a):
          return K0*exp(1j*2*pi/3)
     return K0
