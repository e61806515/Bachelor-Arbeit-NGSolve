...
#
#
def MakeGeometry(H_L, H_M, H_Fe, H_W, Tau, PZ, N_seg):
    ...
    magnets = [WorkPlane(Axes((0,0,0), n=Z, h=X))]*PZ*N_seg   
    d_phi = 360/PZ
    phi_M = d_phi*Tau/2
    phi_seg = d_phi*Tau/N_seg
    for i in range(PZ):
        #if (i*d_phi < 90) or (i*d_phi > 270):
            #magnets = magnets + [WorkPlane(Axes((0,0,0), n=Z, h=X))]
            for j in range(N_seg):
                print(i, j)
                magnets[i*N_seg+j].MoveTo(r_Fe*sin(i*d_phi*pi/180 - pi/180*(phi_M+phi_seg*(j+1))), r_Fe*cos(i*d_phi*pi/180) - pi/180*(phi_M+phi_seg*(j+1)))
                if j == 0:
                    magnets[i*N_seg+j].Arc(r_Fe, -phi_seg).Rotate(90)
                    magnets[i*N_seg+j].Line(H_M).Rotate(90)
                    magnets[i*N_seg+j].Arc((r_Fe + H_M), phi_seg).Rotate(90)
                    magnets[i*N_seg+j].Line(H_M).Rotate(90)
                else:
                    magnets[i*N_seg+j].Arc(r_Fe, -phi_seg).Rotate(90)
                    magnets[i*N_seg+j].Line(H_M).Rotate(90)
                    magnets[i*N_seg+j].Arc((r_Fe + H_M), -phi_seg).Rotate(90)
                    if j==N_seg-1:
                         magnets[i*N_seg+j].Rotate(d_phi)
                magnets[i*N_seg+j] = magnets[i*N_seg+j].Face()
                magnets[i*N_seg+j].maxh = 0.001
                magnets[i*N_seg+j].name = f"magnets_{i}_{j}"
                magnets[i*N_seg+j].col = (0, 0, 1)
...
#geo = OCCGeometry(MakeGeometry(H_L=8e-3, H_M=6e-3, H_Fe=26.19e-3, H_W=9e-3, Tau=3/4, PZ=PZ, N_seg = 1), dim = 3)
mesh = Mesh(OCCGeometry(MakeGeometry(H_L=8e-3, H_M=6e-3, H_Fe=26.19e-3, H_W=9e-3, Tau=3/4, PZ=PZ, N_seg = 1), dim = 3).GenerateMesh(mp=mp))
