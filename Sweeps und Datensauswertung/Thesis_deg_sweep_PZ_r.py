...
n_samples = 60
x_val = [[1,1], [1,20], [20, 20], [20, 40], [40,40], [40, 60], [60,60], [60,80], [80,80], [80,100], [100,100]]
...
with (open(f'sweep_deg_time_{tau}_{nu}.csv', 'w') if tau is 1 and savetime is 1 else nullcontext()) as time_file:
    ...
    for [c_r, c_PZ] in x_val:
        ...
        #Adaptive Meshing
        mesh = Mesh(OCCGeometry(MakeGeometry(H_L=8e-3, H_M=6e-3, maxh_rotor=maxh_rotor, maxh_mag=maxh_mag, r_Fe=c_r*144.5e-3, tau=tau, PZ=c_PZ*PZ, maxh = maxh), dim = 2).GenerateMesh())
        mesh.Curve(3)
        ...
        f = LinearForm(V)
        f += K(x,y,c_PZ*PZ, nu)*test.Trace()*ds("outer")
        ...
        data.append((f'{freq},{c_r},{c_PZ},{losses.real},{losses.real/A},{A}\n'))
        if savetime:
            time_file.write(f'{mesh.ne},{elapsed_time}\n')
        p_values.append(losses.real)
    with open(f'sweep_deg_freq_{freq}_{tau}_{nu}_PZ{PZ}_c_rx144.5mm.csv', 'w') as file:
        for d in data:
            file.write(d)
