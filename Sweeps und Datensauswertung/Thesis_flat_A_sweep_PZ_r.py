...
p_values=[]
i=0
print(f'sweep_flat_Around_{tau}_{nu}_Cr_cPZ.csv')
data = []
for [c_r, c_PZ] in x_val:
        i=i+1
        print(f"Starting {i}th Simulation at f = {freq}, nu = {nu}, c_r = {c_r} and c_PZ = {c_PZ}\n")
        ...
        PZ = c_PZ*8
        r_rot = c_r*144.5e-3
        b = np.pi*((r_rot+d_M)**2- (r_rot)**2)/(PZ*d_M)
        ...
        data.append((f'{freq},{c_r},{c_PZ},{losses.real},{losses.real/A_mag},{A}\n'))
        p_values.append(losses.real)
        print(f"Amags = {A}")
with open(f'sweep_flat_A_freq_{freq}_{tau}_{nu}_PZ{PZ}_c_rx144.5mm.csv', 'w') as file:
        for d in data:
            file.write(d)
